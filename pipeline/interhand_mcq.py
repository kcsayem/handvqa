...
class Posecodes():
    def __init__(self, hand_type, verts, joints,rotation, file_path=None):
    ...
    def scale_and_transform(self, vertices, joints):
        ...
        transform = np.matmul(transform, rotation_matrix(float(self.rotation[0]), self.rotation[1]))
...
if __name__ == '__main__':
    ...
    captions = {}
    benchmark_data = {}
    annot = os.path.join(dataset_path,f"annotations/train/InterHand2.6M_train_MANO_NeuralAnnot.json")
    annot_data = json.load(open(annot))
    capture_ids = os.listdir(os.path.join(dataset_path,f"images/train"))

    train_capture_ids = [f"Capture{i}" for i in range(5,27)]
    if mode == "test":
        capture_ids = [x for x in capture_ids if x not in train_capture_ids]
    elif mode == "train":
        capture_ids = train_capture_ids

    camera_id_rotation = {"cam400053": [75.0,"y"] , "cam400064": [10,"x"]}
    scenes = sorted(os.listdir(os.path.join(dataset_path,f"images/train/Capture24/")))[:40] #4

    data = []
    for capture_id in capture_ids:
        for scene in scenes:
            for camera_id in camera_id_rotation.keys():
                path = glob.glob(os.path.join(dataset_path,
                    f"images/train/{capture_id}/{scene}/{camera_id}/*.jpg"))
                data.extend(path)

    random.shuffle(data)
    output_path = f"output/{dataset}/{args.mode}"
    if not os.path.exists(output_path):
        os.makedirs(output_path)


    for loc in tqdm(data):
        hand_id = loc.split('/')[-1].split('.')[0][5:]
        capture_id = loc.split('/')[-4][7:]
        id = "/".join(loc.split('/')[-7:])
        camera_id = loc.split('/')[-2]
        hand_type = "right"

        pose = torch.tensor(annot_data[capture_id][str(int(hand_id))][hand_type]["pose"]).reshape(1, -1)
        shape = torch.tensor(annot_data[capture_id][str(int(hand_id))][hand_type]["shape"]).reshape(1, -1)

        manolayer = ManoLayer(mano_root='manopth/mano/models', flat_hand_mean=False, root_rot_mode='axisang',
                              joint_rot_mode='axisang', side=hand_type, use_pca=False, ncomps=45)
        vertices, joints = manolayer(pose, shape)
        joints = np.array(joints.squeeze(0))
        vertices = np.array(vertices.squeeze(0))

        #calculate posecodes:
        posecodes = Posecodes(hand_type="right", verts=vertices,joints=joints,rotation=camera_id_rotation[camera_id])
    ...
    with open(os.path.join(output_path, f'{dataset}_{args.mode}_mcq.json'), 'w') as f:
        json.dump(benchmark_data, f)

    print(f"File saved in '{os.path.join(output_path, f'{dataset}_{args.mode}_mcq.json')}'")
